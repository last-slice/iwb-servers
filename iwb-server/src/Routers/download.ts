...
export function downloadRouter(router:any){
    router.post("/download/ready", async (req: any, res: any) => {
        ...
        try{
            iwbManager.sendUserMessage(req.body.user, SERVER_MESSAGE_TYPES.SCENE_DOWNLOAD, {link: (DEBUG ? "http://localhost:3525/" : "https://deployment.dcl-iwb.co/") + "download/" + req.body.user + "/" + req.body.id})
