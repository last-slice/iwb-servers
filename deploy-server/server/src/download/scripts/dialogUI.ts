...
export async function writeDialogPanelUI(location:string, data:any){
    let template = await writeDialogPanel()
    await fs.writeFileSync(location, template);
}

async function writeDialogPanel(){
    let scene = `
...
import { findAndRunAction } from './actions'
...
export function showDialogPanel(value:boolean, aid?:string){
    if(value){
        let sceneItem = scene.ass.find((asset:any)=> asset.aid === aid)
